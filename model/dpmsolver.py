...
def model_wrapper(
    ...
    def model_fn(x, t_continuous):
        ...
        # CFG following CAT4D
            ...
            if (guidance_scale == 1. and guidance_scale2 == 1.) or unconditional_condition is None:
                return noise_pred_fn(x, t_continuous, cond=condition)
            else:
                x_in = torch.cat([x] * 3)
                t_in = torch.cat([t_continuous] * 3)
                full_uncond = {k: v for k, v in unconditional_condition.items()}
                full_uncond['static_latent'] = torch.zeros_like(full_uncond['static_latent'])
                if isinstance(condition, dict):
                    assert isinstance(unconditional_condition, dict)
                    c_in = dict()
                    for k in condition:
                        if isinstance(condition[k], list):
                            c_in[k] = [torch.cat([full_uncond[k][i], unconditional_condition[k][i], condition[k][i]]) for i in range(len(condition[k]))]
                        else:
                            c_in[k] = torch.cat([full_uncond[k], unconditional_condition[k], condition[k]])
                else:
                    c_in = torch.cat([full_uncond, unconditional_condition, condition])
                noise_full_uncond, noise_uncond, noise = noise_pred_fn(x_in, t_in, cond=c_in).chunk(3)
                return noise_full_uncond + guidance_scale * (noise_uncond - noise_full_uncond) + guidance_scale2 * (noise - noise_uncond)
