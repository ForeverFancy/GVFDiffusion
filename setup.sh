...
if [ "$NEW_ENV" = true ] ; then
    conda create -n GVFDiffusion python=3.10
    conda activate GVFDiffusion
...
if [ "$BASIC" = true ] ; then
    pip install pillow imageio imageio-ffmpeg tqdm easydict opencv-python-headless scipy ninja rembg onnxruntime trimesh xatlas pyvista pymeshfix igraph transformers accelerate omegaconf tensorboard timm lpips kiui
...
if [ "$VOX2SEQ" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        mkdir -p /tmp/extensions
        cp -r model/sparse_voxel_diffusion/vox2seq/ /tmp/extensions/vox2seq
