    ...
    def _process_repo(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                logger.error(f"Error in pulling lfs files: {e}")
            ...
            files_with_3d_extension = [
                file
                for file in files
                if any(file.lower().endswith(ext) for ext in FILE_EXTENSIONS) and not os.path.islink(file)
            ]
    ...
    def _parallel_process_repo(cls, args) -> Dict[str, str]:
        ...
        for _ in range(3):
            try:
                ...
                logger.error(f"Error in processing {repo_id}: {e}")
