...
def _render(file_path, sha256, output_dir, num_views, frame_num, gpu_id, overwrite=False, uniform_sampling=False, save_name='renders'):
    output_folder = os.path.join(output_dir, save_name, sha256)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_name', type=str, default='renders', help='Name of the rendered images')
    opt = parser.parse_args(sys.argv[2:])
    opt = edict(vars(opt))

    os.makedirs(os.path.join(opt.output_dir, f'{opt.save_name}'), exist_ok=True)
    ...
    for sha256 in copy.copy(metadata['sha256'].values):
        if os.path.exists(os.path.join(opt.output_dir, f'{opt.save_name}', sha256, 'transforms.json')):
    ...
    # Process objects
    func = partial(_render, output_dir=opt.output_dir, num_views=opt.num_views, frame_num=opt.frame_num, overwrite=opt.overwrite, uniform_sampling=opt.uniform_sampling, save_name=opt.save_name)
